...
(function () {
  ...
  function OnboardWizard({ onBack, onSent, plan }) {
    ...
    // how full each phase's bar is (0–1): past phases full, future empty, the
    // current phase fills by how far through its sub-steps you are — so the
    // progress reads as a percentage that climbs with every Continue.
    const phaseFill = (i) => {
      const cur = STEP_PHASE[step];
      if (cur > i) return 1;
      if (cur < i) return 0;
      const stepsIn = STEP_PHASE.map((p, idx) => (p === i ? idx : -1)).filter((x) => x >= 0);
    ...
    return (<div id="lock"><div className="lock-card" style={{ width: 460, maxWidth: 'calc(100vw - 40px)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 18 }}>
        <svg viewBox="0 0 200 140" width="32" height="22" aria-hidden="true" style={{ display: 'block', flex: 'none' }}>
          <mask id="onbCdSlot"><rect x="0" y="0" width="200" height="140" fill="#fff" /><rect x="60" y="64.5" width="92" height="11" fill="#000" /></mask>
          <g mask="url(#onbCdSlot)"><circle cx="60" cy="70" r="48" fill="var(--ink)" /><path d="M116,22 H136 A48,48 0 0 1 136,118 H116 Z" fill="var(--ink)" /></g>
          <circle cx="112" cy="70" r="6.5" fill={ACC} />
        </svg>
        <span style={{ fontWeight: 800, fontSize: 18, letterSpacing: '-0.01em', color: 'var(--ink)' }}>CurrencyDesk</span>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 20 }}>
        ...
        {PHASES.map((ph, i) => { const active = i <= STEP_PHASE[step]; const pct = Math.round(phaseFill(i) * 100); return (
          <span key={ph} style={{ flex: 1 }}>
            <span style={{ display: 'block', fontFamily: 'var(--f-mono)', fontSize: 8.5, letterSpacing: '0.1em', color: active ? ACC : 'var(--faint)', marginBottom: 4 }}>{ph.toUpperCase()}</span>
            <span style={{ display: 'block', height: 4, borderRadius: 3, background: 'var(--hair)', overflow: 'hidden' }}>
              <span style={{ display: 'block', height: '100%', width: pct + '%', background: ACC, borderRadius: 3, transition: 'width .45s cubic-bezier(.2,.8,.2,1)' }} />
            </span>
          </span>); })}
      </div>
  ...
  function App() {
    ...
    const [deskLocked, setDeskLocked] = useState(false);   // B · session lock (PIN unlock)
    const [handover, setHandover] = useState(false);        // C · operator handover
    ...
    // stage early-returns so closures created on the lock/station screens
    ...
    if (stage === 'lock') return <SignIn
      ...
      onMustChange={(rec, temp, srvPlan, srvUser) => {
        ...
        srvUserRef.current = srvUser || null; setSrvUser(srvUser || null);
        setUser(rec.code || rec.name); setAuthRec(rec); setPwTemp(temp || null); setStage('setpass');
      }}
      onComplete={async (rec, srvPlan, srvUser) => {
        if (rec._adopted) setSettings(s => ({ ...s, employees: [...(s.employees || []), { ...rec, _adopted: undefined }] }));
        if (srvPlan) setSettings(s => ({ ...s, billingPlan: srvPlan }));
        srvUserRef.current = srvUser || null; setSrvUser(srvUser || null);
        setUser(rec.code || rec.name); setAuthRec(rec);
    ...
    if (stage === 'setpass') return <SetPassword staffId={user} current={pwTemp && pwTemp.current} onDone={async () => { setPwTemp(null); await hydrateTenant(); routeAfterAuth(authRec); }} onBack={() => { setPwTemp(null); setStage('lock'); }} />;
    ...
    if (stage === 'station') return <StationPicker branches={branches} station={station} rec={authRec} onBack={() => setStage('lock')} onPick={(st) => { setStation(st); enterDesktop(); }} />;
    ...
    return (<div id="os">
      ...
      {deskLocked && ReactDOM.createPortal(<LockDesk operator={me} onUnlock={() => setDeskLocked(false)}
        onSwitch={() => { setDeskLocked(false); setHandover(true); }}
        onSignout={() => { setDeskLocked(false); logout(); }} />, document.body)}
      {/* C · operator handover — pass the desk to a colleague, on the record */}
            ...
            <div className="mb-power-wrap" ref={opMenuRef}>
              ...
              {opMenu && (
                <div className="mb-menu mb-menu-solid" role="menu">
                  ...
                  <div className="mb-menu-div"></div>
                  <button className="mb-menu-row" onClick={() => { setOpMenu(false); setDeskLocked(true); }}><Ic n="lock" s={16} /> <span className="mb-menu-lbl">Lock desk</span></button>
                  <button className="mb-menu-row" onClick={() => { setOpMenu(false); setHandover(true); }}><Ic n="users" s={16} /> <span className="mb-menu-lbl">Switch operator</span></button>
                  <div className="mb-menu-div"></div>
