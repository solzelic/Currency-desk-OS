...
async function reset(authorized: string[] = [DEMO.branchId]) {
  await pool.query(
    "TRUNCATE ledger_vault_movements,ledger_vault_balances,ledger_operational_cash_movements,ledger_till_counts,ledger_till_count_batches,ledger_till_sessions,ledger_audit_events,ledger_till_balances,ledger_principals CASCADE",
  );
  await pool.query(
