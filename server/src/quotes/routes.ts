...
const postBody=z.object({idempotencyKey:z.string().min(1).max(200)});
...
app.post("/api/quotes/:quoteId/post",async(req,reply)=>{const body=postBody.safeParse(req.body);if(!body.success)return reply.code(400).send({code:"INVALID_REQUEST"});try{const a=await current(req,reply);return a?reply.code(201).send(await service.post(a,(req.params as any).quoteId,body.data.idempotencyKey)):undefined;}catch(e){return fail(reply,e);}});
}
